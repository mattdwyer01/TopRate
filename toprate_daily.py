...
.bt-wrap{{display:grid;grid-template-columns:260px 1fr;gap:16px;align-items:start;}}
...
.bt-ctrl-row label{{font-size:11px;color:#374151;display:flex;justify-content:space-between;align-items:center;}}
.bt-ctrl-row input[type=range]{{width:100%;margin-top:4px;}}
.bt-val{{font-family:'Space Mono',monospace;font-size:11px;color:#f97316;font-weight:700;}}
.bt-kpi-strip{{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-bottom:4px;}}
...
function initBacktest(){{
  ...
  SIG_NAMES.forEach((name,i)=>{{
    const cb=document.createElement('label');
    cb.style.cssText='display:flex;align-items:center;gap:4px;font-size:10px;color:#e2e5ea;cursor:pointer;';
    const inp=document.createElement('input');
    inp.type='checkbox';inp.dataset.sig=name;inp.className='bt-sig-cb';
    inp.checked=btSigs.has(name);
    inp.addEventListener('change',()=>{{btSigs=new Set([...document.querySelectorAll('.bt-sig-cb:checked')].map(c=>c.dataset.sig));runBacktest();}});
    cb.appendChild(inp);
    cb.appendChild(document.createTextNode(SIG_LABELS[i]));
    grid.appendChild(cb);
  }});
  runBacktest();
}}

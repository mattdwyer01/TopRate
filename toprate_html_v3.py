...
/* Error state - a distinct variant of .empty-state for a failed fetch/sync,
   not just an empty result. Rose accent + retry button so a broken feed
   doesn't look like "no races today". */
.empty-state.error {
  background: var(--rose-bg); border: 1px solid var(--rose-line);
}
.empty-state.error .head { color: #991b1b; }
.empty-state.error .sub { color: #b91c1c; font-family: var(--font-body); font-size: 12px; }
.empty-state .retry-btn {
  margin-top: 14px; font-family: var(--font-body); font-size: 11px; font-weight: 700;
  color: #fff; background: var(--rose); border: none; padding: 7px 16px;
  border-radius: var(--radius-sm); cursor: pointer;
}
.empty-state .retry-btn:hover { filter: brightness(0.93); }
...
/* Compact column density (default). Hides the columns that inform a bet
   but rarely decide it; still one click away via the row-detail panel.
   "Full" removes .rt-compact and brings every column back. */
.rt-view-toggle { margin: 0 0 10px 0; }
.race-table.rt-compact th.rt-col-jky, .race-table.rt-compact td.rt-col-jky,
.race-table.rt-compact th.rt-col-trn, .race-table.rt-compact td.rt-col-trn,
.race-table.rt-compact th.rt-col-settles, .race-table.rt-compact td.rt-col-settles,
.race-table.rt-compact th.rt-col-wpjpk, .race-table.rt-compact td.rt-col-wpjpk,
.race-table.rt-compact th.rt-col-wpja, .race-table.rt-compact td.rt-col-wpja,
.race-table.rt-compact th.rt-col-manual, .race-table.rt-compact td.wpr-manual-cell {
  display: none;
}
...
<div class="shell">
  ...
  <section class="section" id="sec-settings">
    ...
    <!-- Score threshold (used by Race tab highlighting) -->
    <div class="settings-card">
      ...
      <div class="setting-row" style="border-top:none;">
        <div>
          ...
          <div class="desc">
            Inactive: the cumulative score this fed was deleted in the
            WPR-only refactor. This control is a leftover and currently
            never highlights anything on the Race tab.
          </div>
        </div>
      ...
      <div class="setting-row">
        <div>
          <strong>WPR book %</strong>
          <div class="setting-help">
            The book the WPR prices should sum to. 100 = fair (mathematically
            true prices). 90 = prices are ~11% longer than fair, so fewer fixed
            prices count as overlays (tighter value threshold). Lower values
            make overlays harder to find; higher values are more generous.
            Default 90.
          </div>
        </div>
        <input type="number" class="setting-input" id="setting-book-pct" value="90" min="50" max="120" step="1">
      </div>
    </div>
...
const defaultSettings = {
  ...
  minStake: 0.25,
  maxStake: 4,
  // WPR book percentage. The softmax prices sum to a 100% book (fair value);
  // dividing each by bookPct/100 lengthens them. Default 90 means the implied
  // book sums to 0.90, so all prices are ~11% longer than fair - fewer fixed
  // prices show as overlays (tighter "value" threshold). Range 50-120.
  wprBookPct: 90,
...
['setting-unit','setting-target','setting-min','setting-max','setting-score-thresh','setting-book-pct'].forEach(id => {
  ...
  el.addEventListener('change', () => {
    ...
    if (id === 'setting-book-pct') {
      // Clamp to a sane range. 50-120 covers fair-to-overround book targets.
      settings.wprBookPct = Math.max(50, Math.min(120, v));
      // WPR prices change - re-render all surfaces that show them.
      if (typeof renderWprSummary === 'function') renderWprSummary();
      if (typeof renderRaceDetail === 'function' && window._currentRaceId) {
        renderRaceDetail(window._currentRaceId);
      }
    }
    saveSettings();
  });
...
document.getElementById('setting-book-pct').value = settings.wprBookPct != null ? settings.wprBookPct : 90;
document.getElementById('unit-display').textContent = '1u = $' + settings.unitDollar;
...
function __initDashboard() {
  ...
  // A shared/bookmarked race link (?tab=race&date=...&race=...) wins over
  // the restored tab, so opening a shared link always lands on that race.
  _rtApplyStateFromUrl();
...
function __bootDashboard() {
  ...
  fetch(url, { cache: 'no-cache' })
    ...
    .catch(function (e) {
      // Surface a clear error rather than a silent blank page. Distinct
      // "error" styling (rose accent, retry button) tells this apart from
      // the neutral "no races today" empty state.
      const host = document.getElementById('race-meetings-list');
      if (host) host.innerHTML = '<div class="empty-state error"><div class="head">' +
        'Could not load race data</div><div class="sub">' +
        (e && e.message ? e.message : 'fetch failed') +
        '</div><button class="retry-btn" onclick="__bootDashboard()">Retry</button></div>';
...
// User-managed: TopRate doesn't surface abandonment until results come in
...
// ── Race URL state ───────────────────────────────────────────────────────
// Selecting a race updates the URL (?tab=race&date=...&race=...) via
// history.pushState (no reload), so a link can be shared/bookmarked
// straight to a race, and the browser back/forward buttons step through
// race selections instead of doing nothing. _rtSyncing guards the popstate
// handler so it only ever renders from the URL, never pushes a new entry.
let _rtSyncing = false;
function _rtPushRaceUrl(raceId, date) {
  if (_rtSyncing) return;
  try {
    const p = new URLSearchParams(window.location.search);
    p.set('tab', 'race');
    if (date) p.set('date', date); else p.delete('date');
    if (raceId) p.set('race', String(raceId)); else p.delete('race');
    history.pushState({ raceId: raceId || null, date: date || null },
      '', window.location.pathname + '?' + p.toString());
  } catch (e) {}
}
function _rtPushMeetingsUrl(date) {
  if (_rtSyncing) return;
  try {
    const p = new URLSearchParams(window.location.search);
    p.set('tab', 'race');
    if (date) p.set('date', date); else p.delete('date');
    p.delete('race');
    history.pushState({ raceId: null, date: date || null },
      '', window.location.pathname + '?' + p.toString());
  } catch (e) {}
}
// Applies whatever the current URL says (deep link on load, or a
// back/forward navigation). Render-only - never pushes a new URL.
function _rtApplyStateFromUrl() {
  let p;
  try { p = new URLSearchParams(window.location.search); } catch (e) { return; }
  if (p.get('tab') !== 'race') return;
  _rtSyncing = true;
  try {
    const date = p.get('date');
    const raceId = p.get('race');
    if (date) currentBrowseDate = date;
    if (raceId && RACES.some(r => String(r.race_id) === String(raceId))) {
      _activateTab('race');
...
window.addEventListener('popstate', _rtApplyStateFromUrl);
...
function todayPickPassesFilters(pick) {
  ...
  // the manual override for the TopRate blind spot - abandonment data
...
function renderMeetingsGrid() {
  ...
  host.querySelectorAll('.mt-race-cell[data-rid]').forEach(cell => {
    cell.addEventListener('click', () => {
      ...
      _rtPushRaceUrl(currentRaceId, currentBrowseDate);
    });
  });
    ...
    btn.addEventListener('click', e => {
      ...
      if (!currentlyAbandoned) {
        const ok = confirm('Mark all races at ' + venue + ' on ' + date +
          ' as abandoned? This will hide their picks from Today.');
        if (!ok) return;
      }
...
function navigateToRace(raceId) {
  ...
  currentRaceId = raceId;
  showRaceDetail(raceId);
  _rtPushRaceUrl(raceId, currentBrowseDate);
...
// trainer, their ratings/strike rates, distance/going history.
...
function _wprRaceModel(race) {
  ...
  if (validIds.length >= 2) {
    ...
    // Book multiplier from settings: 100 = fair (100% book sum), 90 = book sums
    // to 0.90 = prices lengthened by ~11%, fewer overlays / tighter overlay
    // threshold. We divide each fair price by (bookPct/100). Default 90.
    const bookPct = (typeof settings !== 'undefined' && settings.wprBookPct != null)
      ? Math.max(50, Math.min(120, settings.wprBookPct)) : 90;
    const bookMul = bookPct / 100;
    validIds.forEach((id, i) => { price[id] = (sum / exps[i]) / bookMul; });
  }
...
function renderWprSummary() {
  ...
  if (!overlays.length) {
    ...
    const pending = overlays.filter(r => !_isResulted(r));
  ...
  if (!standouts.length) {
    ...
    const pending = standouts.filter(r => !_isResulted(r));
  ...
  ['wpr-list-overlays', 'wpr-list-standouts'].forEach(id => {
    ...
    host.addEventListener('click', (e) => {
      ...
      // Manual FP input click: don't trigger row jump.
      if (e.target.closest('.wpr-fp-manual')) {
        e.stopPropagation();
        return;
      }
...
// ── Race table column density ───────────────────────────────────────────
// Compact hides Jockey/Trainer/Speed/Peak WPR/Avg L3/Manual - the columns
// that inform a decision but rarely drive one. They stay one click away
// (the existing row-click detail panel), and Full brings them all back on
// the table itself for a side-by-side field scan. Defaults to compact.
const RT_COMPACT_KEY = 'toprate_race_table_compact_v1';
function isRaceTableCompact() {
  try {
    const v = localStorage.getItem(RT_COMPACT_KEY);
    return v === null ? true : v === '1';
  } catch (e) { return true; }
}
function setRaceTableCompact(v) {
  try { localStorage.setItem(RT_COMPACT_KEY, v ? '1' : '0'); } catch (e) {}
}
...
function renderRaceDetail(raceId) {
  ...
  stripEl.querySelectorAll('.meeting-tile').forEach(tile => {
    tile.addEventListener('click', () => {
      ...
      if (rid && rid !== String(raceId)) {
        currentRaceId = rid;
        showRaceDetail(rid);
        _rtPushRaceUrl(rid, currentBrowseDate);
      }
    });
  });
  ...
  if (meetingAbandoned) {
    ...
    abandonHtml = '<button type="button" class="rd-abandon-btn" ' +
      ...
      'from Today.">mark abandoned</button>';
  }
  // ── Race confidence ──────────────────────────────────────────────────
  ...
  // timeRanks - within-race rank of speed_rating (u.spd). This was the
  // "Time" voting signal used by the (now-removed) Edge model.
  ...
  const _rtCompact = isRaceTableCompact();
  document.getElementById('rd-runners-table').innerHTML =
    '<div class="subtabs-row rt-view-toggle">' +
      '<button class="subtab' + (_rtCompact ? ' active' : '') + '" data-rt-compact="1">Compact</button>' +
      '<button class="subtab' + (!_rtCompact ? ' active' : '') + '" data-rt-compact="0">Full</button>' +
    '</div>' +
    '<table class="race-table' + (_rtCompact ? ' rt-compact' : '') + '">' +
      '<thead><tr>' +
  ...
  // Wire Compact/Full column-density toggle
  document.querySelectorAll('#rd-runners-table .rt-view-toggle .subtab').forEach(btn => {
    btn.addEventListener('click', () => {
      setRaceTableCompact(btn.dataset.rtCompact === '1');
      renderRaceDetail(raceId);
    });
  });
...
if (backBtn) backBtn.addEventListener('click', () => {
  exitRaceDetail();
  _rtPushMeetingsUrl(currentBrowseDate);
});
...
// Manual FP override - user can enter a placeholder finish position mid-day
// before official results land. Persists per run_id in localStorage. When
// real results arrive, fpCell calls clearManualFp to wipe stale entries
// (auto-overwrite, per user decision).
const MANUAL_FP_KEY = 'toprate_v3_manual_fp';
function _getManualFpMap() {
  try { return JSON.parse(localStorage.getItem(MANUAL_FP_KEY) || '{}'); }
  catch(e) { return {}; }
}
function _saveManualFpMap(m) {
  try { localStorage.setItem(MANUAL_FP_KEY, JSON.stringify(m)); } catch(e) {}
}
function getManualFp(rid) {
  if (rid == null) return null;
  const m = _getManualFpMap();
  const v = m[String(rid)];
  return v != null ? v : null;
}
function setManualFp(rid, val) {
  if (rid == null) return;
  const m = _getManualFpMap();
...
function clearManualFp(rid) {
  if (rid == null) return;
  const m = _getManualFpMap();
  if (m.hasOwnProperty(String(rid))) {
    delete m[String(rid)];
    _saveManualFpMap(m);
  }
}
...
function buildSyncPayload() {
  return {
    ...
    // Crucial for sync because TopRate doesn't surface abandonment until results,

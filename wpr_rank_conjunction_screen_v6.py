"""
wpr_rank_conjunction_screen_v6.py - v5's full 5-signal conjunction, but
with wpr_nett rank replaced by wprp_proj rank (this project's own final
model prediction, already incorporating wpr_nett + ewm5 as BASE plus
ADJ_TERMS - see wpr_projection.py) instead of TopRate's raw external
wpr_nett rating.

wprp_proj already blends wpr_nett and ewm5 together (0.30/0.70, no
calibration - see the base-calc revert this session), so this swap risks
making the "WPR" and "form" legs of the conjunction redundant with each
other (both partly measuring the same underlying signal) rather than
adding independent information - worth checking directly rather than
assuming either way.

Same chronological H1/H2 split, same proportional-stake convention, same
both-directions-positive-ROI bar as v1-v5.
...
def run():
    ...
    df["wpr_rank"] = df.groupby("race_id")["wprp_proj"].rank(ascending=False, method="first")
    ...
    print("=== Full 5-signal conjunction: WPRproj + sect_time + ewm5(form) + jockey + trainer ===")
    ...
    for topn in (1, 2, 3):
        n1, n2, r1, r2, ok = eval_rule(
            ...
            f"WPRproj/sect_time/form top-{topn} AND jockey>=20% AND trainer>=20%")
    ...
    for topn in (1, 2, 3):
        eval_rule(lambda h, n=topn: (h["wpr_rank"] <= n) & (h["sect_time_rank"] <= n) & (h["form_rank"] <= n)
                  ...
                  f"WPRproj/sect_time/form top-{topn} AND jockey>=15% AND trainer>=15%")
    print()
    ...
    for topn in (1, 2, 3):
        eval_rule(lambda h, n=topn: (h["wpr_rank"] <= n) & (h["form_rank"] <= n)
                  & (h["jockey_win_pct_90d"] >= 20) & (h["trainer_win_pct_365d"] >= 20),
                  f"WPRproj/form top-{topn} AND jockey>=20% AND trainer>=20%")
    print()

    print("=== Drop WPRproj (weakest solo signal) - sect_time + form + jockey + trainer ===")
    for topn in (1, 2, 3):

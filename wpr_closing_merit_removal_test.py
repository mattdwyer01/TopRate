"""
wpr_closing_merit_removal_test.py - follow-up to the closing_merit dig
(see chat, Sep 2026): a from-scratch, confound-controlled empirical
check found closing_merit's own premise doesn't hold up (best-vs-worst-
historical-closer quintile spread -0.38 WPR against career average,
t=-5.80, p<0.0001, n=228k - wrong sign, though far smaller than the
first, methodologically-confounded pass suggested). closing_merit was
originally adopted on a real, documented "worse MAE, better strike
rate" tradeoff (same shape as the slope-removal decision) - this tests
whether REMOVING it still holds up on today's data, same adoption-bar
methodology (MAE + ROI, leak-free bidirectional half-split) used for
the passage_risk candidate-addition test.

Same scope limitation as every test tonight since the 10-year era cache
isn't available in this session: ~5-month bidirectional half-split, not
the canonical leave-one-era-out test.
...
def run():
    ...
    CANDIDATE_TERMS = [t for t in BASELINE_TERMS if t != "closing_merit"]  # minus closing_merit
    ...
    print("\n" + "=" * 78)
    print("MAE CHECK: does REMOVING closing_merit improve held-out MAE?")
    print("=" * 78)
    baseline_mae = total_mae["baseline"] / total_n
    candidate_mae = total_mae["candidate"] / total_n
    delta = candidate_mae - baseline_mae
    print(f"baseline (current shipped 10 terms, incl. closing_merit): MAE = {baseline_mae:.4f}")
    print(f"candidate (minus closing_merit):                          MAE = {candidate_mae:.4f}")
    print(f"delta: {delta:+.4f}  ({'IMPROVEMENT from removing it' if delta < 0 else 'WORSE without it'})")
